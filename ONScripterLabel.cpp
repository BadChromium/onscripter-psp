...
extern void initONSCodeToUTF16();
...
static struct FuncLUT{
    ...
    {"saveconfig", &ONScripterLabel::saveconfigCommand},
...
void ONScripterLabel::initSDL()
{
    ...
    if ( script_h.screen_size != ScriptHandler::SCREEN_SIZE_SCRIPT ){
        screen_ratio1 *= 9;
        screen_ratio2 *= 8;
...
void ONScripterLabel::openAudio()
{
    ...
    if ( Mix_OpenAudio( 44100, MIX_DEFAULT_FORMAT, MIX_DEFAULT_CHANNELS, DEFAULT_AUDIOBUF ) < 0 ){
        ...
        fprintf(stderr, "Couldn't open audio device!\n"
                "  reason: [%s].\n", SDL_GetError());
        pspDiagLog("Mix_OpenAudio failed: %s", SDL_GetError());
        ...
        pspDiagLog("Mix_OpenAudio OK freq=%d format=0x%04x channels=%d", freq, format, channels);
...
ONScripterLabel::ONScripterLabel()
{
    ...
    pmf_background_cache.invalidate();
...
int ONScripterLabel::init()
{
    ...
    pspDiagLog("STARTUP script open enter");
    int open_result = open();
    pspDiagLog("STARTUP script open result=%d", open_result);
    if (open_result) return -1;
    

    pspDiagLog("STARTUP SDL enter");
    initSDL();
    pspDiagLog("STARTUP SDL ready");
    ...
    pspDiagLog("STARTUP font enter path=%s",font_file);
    if ( sentence_font.openFont( font_file, screen_ratio1, screen_ratio2 ) == NULL ){
        pspDiagLog("STARTUP font failed path=%s",font_file);
        fprintf( stderr, "can't open font file: %s\n", font_file );
        return -1;
    }

    pspDiagLog("STARTUP font ready; env enter");
    loadEnvData();
    pspDiagLog("STARTUP env ready");
...
void ONScripterLabel::flushDirect( SDL_Rect &rect, int refresh_mode )
{
    ...
    SDL_PixelFormat *sf=accumulation_surface->format,*df=screen_surface->format;
    if (sf->BytesPerPixel==2 && df->BytesPerPixel==2 &&
        sf->Rmask==0xf800 && sf->Gmask==0x07e0 && sf->Bmask==0x001f &&
        df->Rmask==0x001f && df->Gmask==0x07e0 && df->Bmask==0xf800 &&
        sf->Amask==0 && df->Amask==0 &&
        !(accumulation_surface->flags & (SDL_SRCALPHA|SDL_SRCCOLORKEY)) &&
        rect.x>=screen_surface->clip_rect.x && rect.y>=screen_surface->clip_rect.y &&
        rect.x+rect.w<=screen_surface->clip_rect.x+screen_surface->clip_rect.w &&
        rect.y+rect.h<=screen_surface->clip_rect.y+screen_surface->clip_rect.h &&
        rect.x>=0 && rect.y>=0 && rect.x+rect.w<=accumulation_surface->w &&
        rect.y+rect.h<=accumulation_surface->h && rect.x+rect.w<=screen_surface->w &&
        rect.y+rect.h<=screen_surface->h && SDL_LockSurface(accumulation_surface)==0){
        if (SDL_LockSurface(screen_surface)==0){
            pspSwap565Rect((unsigned char *)screen_surface->pixels+rect.y*screen_surface->pitch+rect.x*2,
                          screen_surface->pitch,
                          (unsigned char *)accumulation_surface->pixels+rect.y*accumulation_surface->pitch+rect.x*2,
                          accumulation_surface->pitch,rect.w,rect.h);
...
#endif
    if (!fast_blit) SDL_BlitSurface( accumulation_surface, &rect, screen_surface, &rect );
    ...
    if ( current_font->is_transparent ){
        ...
        if (fmt->Rmask==0xf800 && fmt->Gmask==0x07e0 && fmt->Bmask==0x001f){
            for (int y=0;y<rect.h;++y)
                pspShade565Row((Uint16 *)((unsigned char *)surface->pixels+(rect.y+y)*surface->pitch)+rect.x,
                               rect.w,current_font->window_color[0],current_font->window_color[1],current_font->window_color[2]);
...
int ONScripterLabel::saveEnvData()
{
    file_io_buf_ptr = 0;
