...
extern ONSAudioGeneration ons_music_generation;
extern ONSAudioGeneration ons_wave_generation[ONS_MIX_CHANNELS+ONS_MIX_EXTRA_CHANNELS];
...
protected:
    ...
    bool isCurrentAudioEvent(const SDL_Event &event);
    void flushEvent();
...
private:
    ...
    int pmf_sprite_no;
    int pmf_sprite_last_serial;
    int pmf_sprite_next_delay;
    bool pmf_refresh_only;
    PSPBackgroundCache pmf_background_cache;
    ...
    int startPmfSprite( int no, AnimationInfo *anim );
    void stopPmfSprite( int no );
    int updatePmfSpriteFrame();
    static void pumpPmfDuringDecode(void *context);
    ...
    void alphaBlend( SDL_Surface *mask_surface,
                     int trans_mode, Uint32 mask_value = 255, SDL_Rect *clip=NULL, unsigned mask_gain=1 );
