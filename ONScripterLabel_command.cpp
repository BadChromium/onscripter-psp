...
int ONScripterLabel::saveconfigCommand()
{
    getret_int = saveEnvData();
    if(saveGlovalData()!=0)getret_int=-1;
    return RET_CONTINUE;
}
    ...
    if (script_h.getStringBuffer()[string_buffer_offset] == 0x0a){
        ...
        if (!sentence_font.isLineEmpty() && !new_line_skip_flag){
            ...
            for (int i=0 ; i<indent_offset ; i++){
                current_text_buffer->addBuffer(ONS_FULLWIDTH_SPACE_0);
                current_text_buffer->addBuffer(ONS_FULLWIDTH_SPACE_1);
...
int ONScripterLabel::lspCommand()
{
    ...
    int no = script_h.readInt();
    // Preserve a published frame across same-layer PMF replacement. Loading a
    // transparent surface here exposed the background before the first frame.
    bool retain_pmf_frame = v && pmf_sprite_no == no &&
        sprite_info[no].visible && pmf_sprite_last_serial > 0 &&
        sprite_info[no].trans_mode == AnimationInfo::TRANS_PMF &&
        sprite_info[no].image_surface;
    int old_pmf_x = sprite_info[no].pos.x;
    int old_pmf_y = sprite_info[no].pos.y;
    ...
    retain_pmf_frame = retain_pmf_frame &&
        sprite_info[no].trans_mode == AnimationInfo::TRANS_PMF &&
        sprite_info[no].pos.x == old_pmf_x && sprite_info[no].pos.y == old_pmf_y;
    if ( !retain_pmf_frame ) setupAnimationInfo( &sprite_info[ no ] );
    if ( sprite_info[ no ].trans_mode == AnimationInfo::TRANS_PMF ){
        int started = startPmfSprite(no, &sprite_info[ no ]);
        if ( started < 0 && retain_pmf_frame )
            sprite_info[no].fill( 0, 0, 0, 0 ); // Never conceal failure with stale video.
    }
    if ( sprite_info[no].visible )
        dirty_rect.add( sprite_info[no].pos );
...
int ONScripterLabel::loadgameCommand()
{
    ...
    if ( loadSaveFile( no ) ) return RET_CONTINUE;
        ...
        // Deserialization allocates a PMF surface but does not start its decoder.
        for (int i=0;i<MAX_SPRITE_NUM;++i){
            if(sprite_info[i].trans_mode==AnimationInfo::TRANS_PMF &&
               sprite_info[i].image_name && startPmfSprite(i,&sprite_info[i])<0)
                errorAndExit("Unable to restart the saved movie layer.");
        }
        dirty_rect.fill( screen_width, screen_height );
        flush( refreshMode() );
...
int ONScripterLabel::dwaveCommand()
{
    ...
    if (play_mode == WAVE_PLAY_LOADED){
        onsPlayChannel(ch, wave_sample[ch], loop_flag?-1:0);
    }
    else{
        const char *buf = script_h.readStr();
...
int ONScripterLabel::btnwaitCommand()
{
         ...
         (textbtn_flag && (skip_flag || (draw_one_page_flag && clickstr_state == CLICK_WAIT) || ctrl_pressed_status)) )
    {
        ...
        if (current_button_state.button != 0)
            pspDiagLog("UI_WAIT button=%d text=%d on=%d display=%d next=%d escspc=%d enter=%d",
                current_button_state.button, textbtn_flag, text_on_flag,
                display_mode, next_display_mode, useescspc_flag, getenter_flag);
#endif
