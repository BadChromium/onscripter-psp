...
int ONScripterLabel::doEffect( int effect_no, AnimationInfo *anim, int effect_image )
{
    ...
    if ( effect_counter == 0 ){
        ...
        /* Load mask image */
        if ( effect_no == 15 || effect_no == 18 || effect_no == 20 ){
...
#ifdef PSP
        // A completed mask is no longer used by drawing. Do not retain its
        // full-width pixel/alpha buffers while subsequent audio is decoded.
        // Its image name remains intact for lazy reload on the next effect.
        if (effect->effect == 15 || effect->effect == 18 || effect->effect == 20)
            effect->anim.deleteSurface();
#endif
