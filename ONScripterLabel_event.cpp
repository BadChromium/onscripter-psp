...
{
    long decoded = decodeOggVorbis((OVInfo*)userdata, stream, len, true);
    pspDiagAudioPcm(stream, len, decoded);
    OVInfo *ovi=(OVInfo*)userdata;
    if (decoded == 0 && !ovi->completion_posted){
        SDL_Event event;memset(&event,0,sizeof(event));
        event.type = ONS_SOUND_EVENT;
        event.user.code=ONS_OGG_COMPLETION;
        event.user.data1=(void*)(uintptr_t)ovi->playback_generation;
        if(SDL_PushEvent(&event)==0)ovi->completion_posted=true;
    }
}
...
void ONScripterLabel::flushEventSub( SDL_Event &event )
{
    ...
    if ( event.type == ONS_SOUND_EVENT ){
        ...
        ons_wave_generation[event.user.code].retire();
        if ( wave_sample[event.user.code] ){
            ...
            if (event.user.code == MIX_LOOPBGM_CHANNEL0 && 
                ...
                onsPlayChannel(MIX_LOOPBGM_CHANNEL1, 
...
extern "C" void waveCallback( int channel )
{
    ...
    if(channel<0||channel>=ONS_MIX_CHANNELS+ONS_MIX_EXTRA_CHANNELS)return;
    event.type = ONS_WAVE_EVENT;
    event.user.code = channel;
    event.user.data1=(void*)(uintptr_t)ons_wave_generation[channel].token();
    SDL_PushEvent(&event);
}
...
int ONScripterLabel::eventLoop()
{
    ...
    while ( SDL_WaitEvent(&event) ) {
        ...
        if(!isCurrentAudioEvent(event))continue;
        switch (event.type) {
