...
void ONScripterLabel::searchSaveFile( SaveFileInfo &save_file_info, int no )
{
    ...
    if ( sceIoGetstat(file_name, &buf)<0 ){
        char backup[512];snprintf(backup,sizeof backup,"%s.BAK",file_name);
        if(sceIoGetstat(backup,&buf)<0){save_file_info.valid=false;return;}
    }
...
int ONScripterLabel::loadSaveFile( int no )
{
    ...
    // Join the active movie before deserializing/replacing sprite surfaces.
    stopPmfSprite(-1);
    if ( file_version >= 200 )
