...
void ONScripterLabel::alphaBlend( SDL_Surface *mask_surface,
                                  int trans_mode, Uint32 mask_value, SDL_Rect *clip, unsigned mask_gain )
{
...
{
    if (!pmf_refresh_only) pmf_background_cache.invalidate();
#if defined(PSP)
    if (!pmf_refresh_only) pspPmfLayerInvalidateGpu();
#endif
    if (refresh_mode == REFRESH_NONE_MODE) return;
    if (!pmf_refresh_only) updatePmfSpriteFrame();
    ...
    bool background_restored = false;
#if defined(PSP) && defined(BPP16)
    const bool cache_eligible = pmf_refresh_only && surface==accumulation_surface &&
        clip.x==0 && clip.y==0 && clip.w==surface->w && clip.h==surface->h &&
        surface->format->BytesPerPixel==2;
    if (cache_eligible && SDL_LockSurface(surface)==0){
        background_restored = pmf_background_cache.restore(surface->pixels,surface->pitch,surface->w,surface->h);
        SDL_UnlockSurface(surface);
    }
#endif
...
#if defined(PSP) && defined(BPP16)
        if (cache_eligible && SDL_LockSurface(surface)==0){
            pmf_background_cache.capture(surface->pixels,surface->pitch,surface->w,surface->h);
            SDL_UnlockSurface(surface);
        }
#endif
    }
