...
{
    pspDiagLog("playSound file=%s format=%d audio_open=%d", filename, format, audio_open_flag);
    if ( !audio_open_flag ) return SOUND_NONE;

    long length = script_h.cBR->getFileLength( filename );
    pspDiagLog("playSound file_length=%ld", length);
...
{
    if (!chunk){pspDiagLog("WAVE load failed channel=%d error=%s",channel,Mix_GetError());return -1;}
    ...
    if ( !(format & SOUND_PRELOAD) ){
        int result=onsPlayChannel( channel, wave_sample[channel], loop_flag?-1:0 );
        pspDiagLog("WAVE play channel=%d generation=%u bytes=%u result=%d error=%s",channel,ons_wave_generation[channel].token(),chunk->alen,result,result<0?Mix_GetError():"none");
...
int ONScripterLabel::playOGG(int format, unsigned char *buffer, long length, bool loop_flag, int channel)
{
    ...
    pspDiagLog("playOGG opened=%d", ovi != NULL);
    if (ovi == NULL) return SOUND_OTHER;
    pspDiagLog("playOGG rate=%d channels=%d decoded_length=%ld", rate, channels, ovi->decoded_length);
    
    if (format & SOUND_OGG){
#if defined(PSP)
        // Replacement owns this channel now. Do not retain its old PCM during allocation.
        // Preload keeps its existing load-before-replace semantics.
        if(!(format & SOUND_PRELOAD)){
            Mix_HaltChannel(channel);
            SDL_LockAudio();ons_wave_generation[channel].retire();SDL_UnlockAudio();
            if(wave_sample[channel]){
                Mix_FreeChunk(wave_sample[channel]);
                wave_sample[channel]=NULL;
            }
        }
        Mix_Chunk *chunk=onsDecodePcmChunk(ovi,channel,
            pmf_sprite_no>=0 ? pumpPmfDuringDecode : NULL,this);
#else
        unsigned char *buffer2 = new unsigned char[sizeof(WAVE_HEADER)+ovi->decoded_length];
        long decoded=decodeOggVorbis(ovi, buffer2+sizeof(WAVE_HEADER), ovi->decoded_length, false);
        pspDiagLog("VOICE decoded channel=%d actual=%ld expected=%ld",channel,decoded,ovi->decoded_length);
        if(decoded<=0||decoded>ovi->decoded_length){
            pspDiagLog("VOICE decode failed channel=%d bytes=%ld",channel,decoded);
            closeOggVorbis(ovi);delete[] buffer2;return SOUND_OGG;
        }
        setupWaveHeader(buffer2, channels, rate, decoded);
        Mix_Chunk *chunk = Mix_LoadWAV_RW(SDL_RWFromMem(buffer2, sizeof(WAVE_HEADER)+decoded), 1);
        delete[] buffer2;
#endif
        closeOggVorbis(ovi);

        int played=playWave(chunk, format, loop_flag, channel);
        pspDiagLog("VOICE submit channel=%d result=%d",channel,played);

        return SOUND_OGG;
    }

    ovi->completion_posted=false;
    ovi->playback_generation=ons_music_generation.begin();
    music_ovi = ovi;
...
int ONScripterLabel::playMPEG( const char *filename, bool click_flag )
{
    int ret = 0;
#if defined(PSP)
    int result = pspPmfPlayVideoOnce(filename, click_flag, screen_surface);
    if (result < 0)
        fprintf(stderr, "PSP PMF playback failed: %d\n", result);
    return 0;
#elif !defined(MP3_MAD)
