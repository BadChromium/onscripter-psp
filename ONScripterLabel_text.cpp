...
static unsigned short onsTextCode( const char *x )
{
    ...
    if ( !onsIsTwoByteLead( p[0] ) || !onsIsTwoByteTrail( p[1] ) ) return 0;
    return (unsigned short)((p[0] << 8) | p[1]);
}

#define ONS_TEXT_CODE(x) onsTextCode( x )
#define IS_KINSOKU(x)	\
        ( ONS_TEXT_CODE(x) == 0xa1a2 || ONS_TEXT_CODE(x) == 0xa1a3 || \
          ONS_TEXT_CODE(x) == 0xa1ad || ONS_TEXT_CODE(x) == 0xa1b1 || \
          ONS_TEXT_CODE(x) == 0xa1b3 || ONS_TEXT_CODE(x) == 0xa1b5 || \
          ONS_TEXT_CODE(x) == 0xa1b7 || ONS_TEXT_CODE(x) == 0xa1b9 || \
          ONS_TEXT_CODE(x) == 0xa1bb || ONS_TEXT_CODE(x) == 0xa1bf || \
          ONS_TEXT_CODE(x) == 0xa3a1 || ONS_TEXT_CODE(x) == 0xa3a9 || \
          ONS_TEXT_CODE(x) == 0xa3ac || ONS_TEXT_CODE(x) == 0xa3ae || \
          ONS_TEXT_CODE(x) == 0xa3ba || ONS_TEXT_CODE(x) == 0xa3bb || \
          ONS_TEXT_CODE(x) == 0xa3bf || ONS_TEXT_CODE(x) == 0xa3dd )
#define IS_ROTATION_REQUIRED(x)	( !IS_TWO_BYTE(*(x)) )
...
void ONScripterLabel::drawGlyph( SDL_Surface *dst_surface, FontInfo *info, SDL_Color &color, char* text, int xy[2], bool shadow_flag, AnimationInfo *cache_info, SDL_Rect *clip, SDL_Rect &dst_rect )
{
    ...
    if (IS_TWO_BYTE(text[0])){
        ...
        unicode = convONSCodeToUTF16( index );
    }
    else{
        unicode = convONSSingleByteToUTF16( ((unsigned char*)text)[0] );
    }
