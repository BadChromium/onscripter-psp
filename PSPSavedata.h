#ifndef PSP_SAVEDATA_H
#define PSP_SAVEDATA_H
...
static int onsPspSavePath(const char *archive,const char *name,char *out,size_t cap){
...
static int onsPspAtomicWrite(const char *path,const void *data,size_t size){
 ...
 return onsPspCommit(path,tmp);
}

static int onsPspMkdir(const char *path){
...
static int onsPspInstall(const char *source,const char *dest,long limit=65536){
 ...
 return onsPspCommit(dest,tmp);
}
static int onsPspPrepare(const char *archive,const char *path,bool refreshMetadata=true){
 ...
 const char *folder=strrchr(dir,'/');if(!folder || strncmp(++folder,onsPspGameId,9))return -1;
 ...
 if(onsPspMkdir(root)!=0 || onsPspMkdir(dir)!=0)return -1;
 ...
 if(existing)fclose(existing);else if(onsPspInstall(source,dest)!=0)return -1;
