...
static int readScriptHeaderNumber( char **buf )
{
    int value = 0;
    while ( **buf >= '0' && **buf <= '9' )
...
ScriptHandler::ScriptHandler()
{
    ...
    script_width = 640;
    script_height = 480;
    script_variable_range = VARIABLE_RANGE;
...
int ScriptHandler::getStringFromInteger( char *buffer, int no, int num_column )
{
    ...
    for (i=0 ; i<num_space ; i++){
        buffer[c++] = ONS_FULLWIDTH_SPACE_0;
        buffer[c++] = ONS_FULLWIDTH_SPACE_1;
    }
    if (num_minus == 1){
        buffer[c++] = ONS_FULLWIDTH_MINUS_0;
        buffer[c++] = ONS_FULLWIDTH_MINUS_1;
    }
    c = (num_column-1)*2;
    for (i=0 ; i<num_digit ; i++){
        int digit = no % 10;
        buffer[c]   = ONS_FULLWIDTH_DIGIT_0;
        buffer[c+1] = ONS_FULLWIDTH_DIGIT_1(digit);
        no /= 10;
        c -= 2;
    }
...
{
    char magic[5] = {0x79, 0x57, 0x0d, (char)0x80, 0x04 };
    int  magic_counter = 0;
...
int ScriptHandler::readScript( char *path )
{
    ...
    if ( script_buffer[0] == ';' && *buf == '$' ){
        buf++;
        while ( *buf != 0x0a && *buf != '\0' ){
            char key = *buf++;
            if ( key >= 'a' && key <= 'z' ) key += 'A' - 'a';
            if ( key == 'V' ){
                script_variable_range = readScriptHeaderNumber( &buf );
            }
            else if ( key == 'G' ){
                global_variable_border = readScriptHeaderNumber( &buf );
                if ( global_variable_border < 0 ) global_variable_border = 0;
                if ( global_variable_border > VARIABLE_RANGE )
                    global_variable_border = VARIABLE_RANGE;
            }
            else if ( key == 'S' ){
                int w = readScriptHeaderNumber( &buf );
                int h = 0;
                if ( *buf == ',' ){
                    buf++;
                    h = readScriptHeaderNumber( &buf );
                }
                if ( w > 0 && h > 0 ){
                    script_width = w;
                    script_height = h;
                    screen_size = SCREEN_SIZE_SCRIPT;
                }
            }
            else{
