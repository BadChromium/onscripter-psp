...
#define SWAPINT(x) (((x)<<24) | (((u32)(x)) >> 24) | (((x) & 0x0000FF00) << 8) | (((x) & 0x00FF0000) >> 8))
...
static int avcodec_module_owned = 0;
static int mpeg_module_owned = 0;

static void pspavReleaseVideoModules(void) {
    if (mpeg_module_owned) {
        pspDiagLog("MPEG unload rc=0x%08x", (unsigned int)sceUtilityUnloadModule(PSP_MODULE_AV_MPEGBASE));
        mpeg_module_owned = 0;
    }
    if (avcodec_module_owned) {
        pspDiagLog("AVCODEC unload rc=0x%08x", (unsigned int)sceUtilityUnloadModule(PSP_MODULE_AV_AVCODEC));
        avcodec_module_owned = 0;
    }
}
...
int pspavInit(sceMpegRingbufferCB RingbufferCallback) {
    ...
    int status = 0;
    // MPEGBASE needs AVCODEC even when the movie has no audio stream.
    int codec_result = sceUtilityLoadModule(PSP_MODULE_AV_AVCODEC);
    pspDiagLog("AVCODEC module rc=0x%08x", (unsigned int)codec_result);
    avcodec_module_owned = codec_result == 0;
    if (codec_result < 0 && (unsigned int)codec_result != 0x80111102u)
        return codec_result;
    if (playAudio){
    ...
    int module_result = sceUtilityLoadModule(PSP_MODULE_AV_MPEGBASE);
    status |= module_result;
    pspDiagLog("MPEG module rc=0x%08x", (unsigned int)module_result);
    mpeg_module_owned = module_result == 0;
    if (module_result < 0 && (unsigned int)module_result != 0x80111102u) {
        pspavReleaseVideoModules();
    ...
    res = sceMpegInit();
    pspDiagLog("MPEG init rc=0x%08x", (unsigned int)res);
    if (res < 0) {
        pspavReleaseVideoModules();
        return res;
    }
    ...
    pspDiagLog("MPEG ringbuffer rc=0x%08x size=%d ptr=%p", (unsigned int)res, m_RingbufferSize, m_RingbufferData);
    if (res < 0) return res;
    
    res = sceMpegCreate(&m_Mpeg, m_MpegMemData, m_MpegMemSize, &m_Ringbuffer, BUFFER_WIDTH, 0, 0);
    pspDiagLog("MPEG create rc=0x%08x size=%d ptr=%p", (unsigned int)res, m_MpegMemSize, m_MpegMemData);
    if (res < 0) return res;
...
void pspavLoad() {
    MPEGcounter = 0;
    MPEGstart = 0;
    int header_result = ParseHeader();
    pspDiagLog("MPEG header rc=%d offset=%d size=%d timestamp=%d", header_result, m_MpegStreamOffset, m_MpegStreamSize, m_iLastTimeStamp);
    ...
    pspDiagLog("MPEG initAu rc=0x%08x stream=%p es=%p", (unsigned int)retVal, m_MpegStreamAVC, m_pEsBufferAVC);
...
int pspavPlay(){
    ...
    retVal = InitReader();
    pspDiagLog("InitReader rc=%d thread=0x%08x", retVal, (unsigned int)Reader.m_ThreadID);
    if (retVal < 0)
    {
        fail++;
        goto exit_reader;
    ...
    retVal = InitVideo();
    pspDiagLog("InitVideo rc=%d thread=0x%08x", retVal, (unsigned int)Video.m_ThreadID);
    if (retVal < 0)
    {
        fail++;
        goto exit_video;
    ...
    retVal = InitDecoder();
    pspDiagLog("InitDecoder rc=%d thread=0x%08x", retVal, (unsigned int)Decoder.m_ThreadID);
    if (retVal < 0)
    {
        fail++;
        goto exit_decoder;
    ...
    if (playAudio && (!playAT3 || (playAT3 && !at3_thread_started))){
...
static int pspavRepeatVideoPasses(int repeat)
{
    int result;
    for (;;) {
        result = pspavPlay();
        if (!repeat || result < 0 || !work ||
            av_callbacks->getPadState() == PAD_USER_CANCEL) break;
        result = sceIoLseek(pspavfd, MPEGstart, PSP_SEEK_SET);
        if (result < 0) break;
        MPEGcounter = MPEGstart;
        mps_header_injected = 0;
        result = sceMpegInitAu(&m_Mpeg, m_pEsBufferAVC, &m_MpegAuAVC);
        if (result < 0) break;
        pspDiagLog("PMF loop rewind: retained file and MPEG context");
    }
    return result;
}

static int pspavPlayVideoFileMode(const char* path, PSPAVCallbacks* callbacks, int repeat){

    if (!callbacks) return -1;

    pspavfd = sceIoOpen(path, PSP_O_RDONLY, 0777);
    pspDiagLog("PMF open path=%s result=%d hex=0x%08x", path, pspavfd, (unsigned int)pspavfd);
    if (pspavfd < 0) return -2;

    char* ext = strrchr(path, '.');
    is_mps = (ext && strcasecmp(ext, ".mps") == 0) ? 1 : 0;
    ...
    int result = pspavInit(RingbufferCallbackFromFile);
    if (result >= 0){
        pspavLoad();
        work = 1;
        result = pspavRepeatVideoPasses(repeat);
        pspavShutdown();
    }
...
int pspavPlayVideoFileOnce(const char* path, PSPAVCallbacks* callbacks){
    return pspavPlayVideoFileMode(path,callbacks,0);
}

int pspavPlayVideoFileLoop(const char* path, PSPAVCallbacks* callbacks){
    return pspavPlayVideoFileMode(path,callbacks,1);
}
