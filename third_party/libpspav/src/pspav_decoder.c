...
int T_Decoder(SceSize _args, void *_argp)
{
    ...
    for (;;)
    {
        ...
        if (playAudio && D->Audio->m_iFullBuffers < D->Audio->m_iNumBuffers)
        {
        ...
        if (D->Video->m_iFullBuffers < D->Video->m_iNumBuffers)
        {
            ...
            else
            {
                ...
                if (iVideoStatus == 1)
                {
                    ...
                    D->Video->m_iBufferTimeStamp[D->Video->m_iPlayBuffer] =
                        pspavPresentationTimestamp(m_iVideoCurrentTimeStamp,m_iVideoLastTimeStamp,av_callbacks->publishTexture != NULL);
    ...
    pspDiagLog("DECODER exit rc=0x%08x decoded_frames=%d timestamp=%d last_timestamp=%d reader_status=%d", (unsigned int)retVal, videoFrameCount, m_iVideoCurrentTimeStamp, D->m_iLastTimeStamp, D->Reader->m_Status);
