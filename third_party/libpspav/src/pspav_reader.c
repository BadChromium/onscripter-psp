    ...
    for (;;)
    {
        ...
        if (iFreePackets > 0)
        {
            ...
            if (D->m_TotalBytes < D->m_StreamSize)
            {
                ...
                if (iPackets < 0)
                {
                    pspDiagLog("READER ringbufferPut failed rc=0x%08x", (unsigned int)iPackets);
        ...
        if (D->m_TotalBytes >= D->m_StreamSize)
        {
            // On the single-core PSP, prevent an abort from arriving between
            // this check and store. No I/O, waits or logging inside this section.
