...
SceInt32 AVSyncStatus(DecoderThreadData* D)
{
    ...
    if(!playAudio || D->Audio->m_iFullBuffers == 0 || D->Video->m_iFullBuffers == 0) return 1;
...
int RenderFrame(DecoderThreadData* D)
{
    ...
    int timestamp = D->Video->m_iBufferTimeStamp[D->Video->m_iPlayBuffer];

    if (av_callbacks->publishTexture)
        return av_callbacks->publishTexture(tex, dx, dy, timestamp);
...
int T_Video(SceSize _args, void *_argp)
{
    ...
    for (;;)
    {
        ...
        if (av_callbacks->directTimestampPacing)
            sceKernelDelayThread(1);
        else if (playAT3 || !playAudio)
            sceKernelDelayThread(10000);
        else
            sceKernelDelayThread(0);
    }
