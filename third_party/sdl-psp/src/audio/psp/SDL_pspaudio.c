...
static void PSPAUD_PlayAudio(_THIS)
{
	...
	int output_result;
	pspDiagAudioHardwarePcm(mixbuf, this->spec.size);

	if (this->spec.channels == 1) {
		output_result = sceAudioOutputBlocking(this->hidden->channel, PSP_AUDIO_VOLUME_MAX, mixbuf);
	} else {
		output_result = sceAudioOutputPannedBlocking(this->hidden->channel, PSP_AUDIO_VOLUME_MAX, PSP_AUDIO_VOLUME_MAX, mixbuf);
	}

	pspDiagAudioOutput(output_result);
...
static int PSPAUD_OpenAudio(_THIS, SDL_AudioSpec *spec)
{
	...
	pspDiagLog("AUDIO hardware reserve channel=%d hex=0x%08x samples=%u channels=%u freq=%d", this->hidden->channel, (unsigned int)this->hidden->channel, spec->samples, spec->channels, spec->freq);
	if (this->hidden->channel < 0) {
